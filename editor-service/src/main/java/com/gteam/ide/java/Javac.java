...
public class Javac {
     ...
     * @param srcCodes
     *            . List of strings.
     * @return List<ClassInfo>, className is fully qualified, and byte[] contains the
     *         bytecode of the class.
     * @throws java.io.IOException
     */
    public static CompilationResult compile(List<String> srcCodes) throws IOException {

        List<SourceInfo> srcInfos = getSourceInfos(srcCodes);

        File rootDir = getTmpDir(); // something like "/tmp/blah$2348983948"
        ...
        JavaCompliationResult compilationResult = new JavaCompliationResult();
        compiler.run(null, compilationResult.outputStream(), compilationResult.errorStream(), args);
        if(compilationResult.hasErrors() == Boolean.FALSE) {
            List<ClassInfo> classInfos = new ArrayList<ClassInfo>();
            addClasses(classInfos, "", classDir);
            deleteDir(rootDir);
    ...
    private static File getTmpDir() throws IOException {
        ...
        Random r = new Random();
        String name = tmpDirName + "gteam$" + r.nextLong();
